...
# Recurrence pattern: 1->2->3->1->2->3 (all layers each pass).
# One sinusoidal step embedding per full pass differentiates recurrent iterations.
# Push/pop skip connections: encoder passes push, decoder passes pop in reverse.
...
class Hyperparameters:
    ...
    num_recurr = int(os.environ.get("NUM_RECURR", 6))
...
class GPT(nn.Module):
    def __init__(
        ...
        self.num_encoder_passes = num_recurr // 2
        self.num_decoder_passes = num_recurr - self.num_encoder_passes
        self.tied_embed_init_std = tied_embed_init_std
        self.logit_softcap = logit_softcap
        self.tok_emb = nn.Embedding(vocab_size, model_dim)
        # One sinusoidal embedding per full recurrent pass — tells every layer which iteration it's in.
        half = model_dim // 2
        positions = torch.arange(num_recurr, dtype=torch.float32).unsqueeze(1)
        freqs = torch.exp(torch.arange(half, dtype=torch.float32) * (-math.log(10000.0) / half))
        emb = torch.zeros(num_recurr, model_dim)
        emb[:, 0::2] = torch.sin(positions * freqs)
        emb[:, 1::2] = torch.cos(positions * freqs)
        self.register_buffer("step_emb", emb)
        # One skip weight per encoder pass (popped in reverse at start of each decoder pass).
        num_skips = min(self.num_encoder_passes, self.num_decoder_passes)
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        # Encoder passes: inject step embedding, run all layers, push state.
        skips: list[Tensor] = []
        for p in range(self.num_encoder_passes):
            x = x + self.step_emb[p][None, None, :]
            for i in range(self.num_layers):
                x = self.blocks[i](x, x0)
            skips.append(x)

        # Decoder passes: pop skip in reverse (mirrored), inject step embedding, run all layers.
        skip_idx = 0
        for p in range(self.num_decoder_passes):
            if skips:
                x = x + self.skip_weights[skip_idx].to(dtype=x.dtype)[None, None, :] * skips.pop()
                skip_idx += 1
            x = x + self.step_emb[self.num_encoder_passes + p][None, None, :]
            for i in range(self.num_layers):
                x = self.blocks[i](x, x0)
...
def main() -> None:
    ...
    log0(f"num_layers:{args.num_layers} num_recurr:{args.num_recurr} encoder_passes:{base_model.num_encoder_passes} decoder_passes:{base_model.num_decoder_passes}")
